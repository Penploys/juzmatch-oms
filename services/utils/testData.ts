...
export class testDataHelper {
      ...
      if (count > 0) {
        if (subjectContains) {
          for (let i = 0; i < count; i++) {
            ...
            const isMatch =
              subjectContains instanceof RegExp ? subjectContains.test(subject) : subject.includes(subjectContains)
  ...
  async clickLinkByText(mailFrame: ReturnType<Page['frameLocator']>, text: RegExp | string) {
    ...
    const [newPage] = await Promise.all([this.context.waitForEvent('page'), link.click()])
